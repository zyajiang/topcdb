...
// Simple8b is 64bit word-sized Simple8bEncoder that packs multiple integers into a single word using
...
// Simple8bEncoder converts a stream of unsigned 64bit integers to a compressed byte slice.
type Simple8bEncoder struct {
...
// NewSimple8bEncoder returns an Simple8bEncoder able to convert uint64s to compressed byte slices
func NewSimple8bEncoder() *Simple8bEncoder {
	return &Simple8bEncoder{
		buf:   make([]uint64, 240),
		b:     make([]byte, 8),
		bytes: make([]byte, 128),
	}
...
func (e *Simple8bEncoder) SetValues(v []uint64) {
	e.buf = v
	e.t = len(v)
	e.h = 0
	e.bytes = e.bytes[:0]
...
func (e *Simple8bEncoder) Write(v uint64) error {
	if e.t >= len(e.buf) {
...
func (e *Simple8bEncoder) flush() error {
	if e.t == 0 {
		return nil
	}
...
func (e *Simple8bEncoder) Bytes() ([]byte, error) {
	for e.t > 0 {
		if err := e.flush(); err != nil {
			return nil, err
		}
...
// EstimatedSize returns the number of bytes that have been written to the Simple8bEncoder
func (e Simple8bEncoder) EstimatedSize() int {
	return len(e.bytes)
}

// Simple8bDecoder converts a compressed byte slice to a stream of unsigned 64bit integers.
type Simple8bDecoder struct {
	bytes []byte
	buf   [240]uint64
	i     int
	n     int
...
// NewSimple8bDecoder returns a Simple8bDecoder from a byte slice
func NewSimple8bDecoder(b []byte) *Simple8bDecoder {
	return &Simple8bDecoder{
		bytes: b,
	}
}
...
func (d *Simple8bDecoder) Next() bool {
	d.i += 1
...
func (d *Simple8bDecoder) SetBytes(b []byte) {
	d.bytes = b
	d.i = 0
	d.n = 0
}
...
// value.
func (d *Simple8bDecoder) Read() uint64 {
	v := d.buf[d.i]
	return v
}

func (d *Simple8bDecoder) read() {
	if len(d.bytes) < 8 {
		return
	}
