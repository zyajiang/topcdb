...
func benchmarkCompress(outPath, dataset, path, algo string, errbound float64) error {
	...
	if dl.ReadDataset(dataset, path) != nil {
		return err
	}
	...
	tMax := int64(7200 * timeDelta)

	if err, _ := tb.selectTestThroughput(tMin, tMax, m1, m2); err != nil {
		return err
	}
...
func (b *CompressBenchmark) selectTestCorrect(dl *tsdb.DatasetLoader, tMin int64, tMax int64, matchers ...*labels.Matcher) error {
	...
	for _, sample := range dl.Samples[col] {
...
func (b *CompressBenchmark) selectTestThroughput(tMin int64, tMax int64, matchers ...*labels.Matcher) (error, int) {
	total := 0

	// 1. 直接从 DB 获取一个包含 Head 块和所有磁盘块的顶级 Querier。
	sq, err := b.storage.Querier(tMin, tMax)
	if err != nil {
		return err, 0
	}
	defer sq.Close()
