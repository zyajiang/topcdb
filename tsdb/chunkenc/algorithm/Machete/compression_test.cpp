...
static inline ssize_t machete_decompress_lorenzo1_hybrid(uint8_t* input , ssize_t size, double* output, double error) {
        return machete_decompress<lorenzo1,huffman>(input, size, output);
}
...
struct {
        ...
        { "Machete",    Type::Lossy,    machete_compress<lorenzo1, huffman>,     machete_decompress_lorenzo1_hybrid,     empty},
...
struct {
        ...
        { "Voltage",     "./example_data/HouseholdVoltage"   , 1E-2},
...
// #define DEBUG_LAST_FAILED
        ...
        for (int i = 0; bsize_list[i] != EOL; i++) {
                ...
                for (int j = 0; dataset_list[j] != EOL; j++) {
                        test_dataset(dataset_list[j], 20000);
